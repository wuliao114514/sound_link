...
impl DelayBuffer {
    pub fn new(delay_ms: u32, sample_rate: u32, channels: usize) -> Self {
        let delay_samples = ((delay_ms as f64 * sample_rate as f64 / 1000.0).round() as usize
            * channels)
            .max(channels);
    ...
    pub fn set_delay(&mut self, delay_ms: u32, sample_rate: u32) {
        let new_delay_samples = ((delay_ms as f64 * sample_rate as f64 / 1000.0).round() as usize
            * self.channels)
            .max(self.channels);
